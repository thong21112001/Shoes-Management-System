...
AS 
BEGIN
	SELECT CateId,NameCate FROM dbo.tblCategory WHERE NameCate LIKE '%' + @name + '%'
END
GO
--> End Category--
...
AS 
BEGIN
	SELECT ProdId,NameSP,Price,Image,CateId,SupId,Quantity FROM dbo.tblProduct WHERE NameSP LIKE '%' + @name + '%'
END
GO
...
AS 
BEGIN
	SELECT UsId,UserName,Password,TypeAccount,DisplayName,Email FROM dbo.tblAccount WHERE Email LIKE '%' + @name + '%'
END
GO
--> End Account--
...
AS 
BEGIN
	SELECT SupId,SupName,SupAddress,PhoneNumber,Email FROM dbo.tblSupplier WHERE PhoneNumber LIKE '%' + @name + '%'
END
GO
--> End Supplier--
...
AS 
BEGIN
	SELECT CusId,Name,Address,PhoneNumber FROM dbo.tblCustomer WHERE PhoneNumber LIKE '%' + @name + '%'
END
GO
--> End Customers--