...
CREATE PROC USP_GetQuantitySP
@idSP int
AS 
BEGIN
	SELECT * FROM dbo.tblProduct WHERE ProdId = @idSP
END
GO
--> End Product--
...
@usid int,@cusid int,@dateTT Datetime,@total float,@status int
AS
BEGIN
	INSERT INTO dbo.tblBill VALUES(@usid,@cusid,@dateTT,@total,@status)
END
GO
...
AS 
BEGIN
	Update dbo.tblBill Set Status = 0 Where BillId = @id
END
GO
...
AS
BEGIN
	SELECT b.BillId, c.Name, DateOfPayment, TotalPrice
	FROM dbo.tblBill b, dbo.tblCustomer c
	WHERE b.CusId = c.CusId AND c.PhoneNumber LIKE '%' + @name + '%'
END
GO
--> End tblBill--
...
@idBill int,@proid int,@quantity int,@price float,@status int
AS
BEGIN
	INSERT INTO dbo.tblBillInfo VALUES(@idBill,@proid,@quantity,@price,@status)
	Update dbo.tblProduct Set Quantity = Quantity - @quantity Where ProdId = @proid and Status = 1
END
GO
...
CREATE PROC ListOfCTBillInfo
@id int
AS
BEGIN
	SELECT p.NameSP, bi.Quantity, p.Price
	FROM dbo.tblBillInfo bi, tblProduct p
	Where p.ProdId = bi.ProdId and p.Price = bi.Price and bi.BillId = @id
END
GO
--> End tblBillInfo--
...
----- CREATE PROCEDDURE Doanh Thu Bán Hàng -----
CREATE PROC GetTotalPrice
AS
BEGIN
	SELECT SUM(TotalPrice) from dbo.tblBill
END
GO

CREATE PROC GetTotalShoes
AS
BEGIN
	SELECT SUM(Quantity) from dbo.tblBillInfo
END
GO

CREATE PROC GetTotalCustomer
AS
BEGIN
	SELECT COUNT(*) from dbo.tblCustomer Where Status = 1
END
GO

CREATE PROC GetTotalSupplier
AS
BEGIN
	SELECT COUNT(*) from dbo.tblSupplier Where Status = 1
END
GO

Create Proc GetTotalDate
@checkin date,@checkout date
AS
Begin
	--Câu truy vấn hiển thị danh sách
	Select B.BillId AS [Số Hóa Đơn],CUS.Name,B.TotalPrice AS [Tổng Tiền],DateOfPayment AS [Ngày Hóa Đơn]
	from tblBill AS B, tblCustomer AS CUS
	Where DateOfPayment  >= @checkin and DateOfPayment <= @checkout and CUS.CusId = B.CusId
End
Go

Create Proc Get7Date
@checkin date,@checkout date
AS
Begin
	--Câu truy vấn hiển thị danh sách
